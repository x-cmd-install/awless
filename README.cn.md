# awless

[English version](./README.md)

A Mighty CLI for AWS

[![x-cmd/install — awless Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/awless.svg?lang=zh)](https://x-cmd.com/install/awless)

## 安装

```sh
x install awless
```

## 代码洞察

合计: **1,500,944** 行代码（覆盖前 5 种语言、共 **3268** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Go | 880,464 | 537,305 | 201,602 | 2531 |
| Json | 598,098 | 0 | 221 | 655 |
| AssemblyGAS | 8,150 | 666 | 915 | 50 |
| Xml | 4,942 | 3 | 87 | 2 |
| Html | 3,352 | 85 | 336 | 30 |

## OpenSSF Scorecard 评分

总评分: **2.1 / 10**

评分最低的几项:

- **Packaging** (-1/10) — packaging workflow not detected
- **Dangerous-Workflow** (-1/10) — no workflows found
- **Token-Permissions** (-1/10) — No tokens found

## 源代码

- **上游仓库**: <https://github.com/wallix/awless>
- **官网**: <http://awless.io/>
- **许可证**: Apache-2.0

## 发布

- **最新版本**: `v0.1.11` (2018-06-21)
- **最近提交**: 2018-12-10
- **Release 含资产**: 5 个

## 流行度

- **Star**: 4,956 · **Fork**: 258 · **开放 issue**: 269 · **贡献者**: 14

## 累计统计

- **发布数**: 24 · **已合并 PR**: 22 · **开放 PR**: 4 · **已关闭 issue**: 154 · **开放 issue**: 115 · **提交数**: 1340

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-24 | 0 | 0 | 0 | 0 | 0 | 0 |
| last60d | 2026-07-25 | 0 | 0 | 0 | 0 | 0 | 0 |
| 90d | 2026-06-25 | 0 | 0 | 0 | 0 | 0 | 0 |
| last180d | 2026-03-27 | 0 | 0 | 0 | 0 | 0 | 0 |
| 360d | 2025-09-28 | 0 | 0 | 0 | 0 | 0 | 0 |
| last720d | 2024-10-03 | 0 | 0 | 0 | 0 | 0 | 0 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [awless-darwin-amd64.tar.gz](https://github.com/wallix/awless/releases/download/v0.1.11/awless-darwin-amd64.tar.gz) | 8.7 MiB | `native/darwin/x64` |
| [awless-linux-386.tar.gz](https://github.com/wallix/awless/releases/download/v0.1.11/awless-linux-386.tar.gz) | 7.7 MiB | `native/unknown` |
| [awless-linux-amd64.tar.gz](https://github.com/wallix/awless/releases/download/v0.1.11/awless-linux-amd64.tar.gz) | 8.1 MiB | `native/linux/x64` |
| [awless-windows-386.zip](https://github.com/wallix/awless/releases/download/v0.1.11/awless-windows-386.zip) | 7.8 MiB | `native/win/x64` |
| [awless-windows-amd64.zip](https://github.com/wallix/awless/releases/download/v0.1.11/awless-windows-amd64.zip) | 8.2 MiB | `native/win/x64` |

## 改进这些数据

awless 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `awless` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/awless.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260923.yml` · 2026-09-23T04:02:26Z._
